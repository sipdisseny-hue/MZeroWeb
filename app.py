...
TEXTOS = {
    "es": {
        ...
        "aviso_login_eval": (
            "Debes iniciar sesión en el sidebar para acceder al módulo de"
            " evaluaciones."
        ),
        "profesor": "Profesor",
        "curso": "Curso",
        ...
        "subt_puntuacion": (
            "Puntuación (1=Insuficiente, 3=Suficiente, 5=Excelente)"
        ),
    ...
    "ca": {
        ...
        "aviso_login_eval": (
            "Has d'iniciar sessió al sidebar per accedir al mòdul d'avaluacions."
        ),
        "profesor": "Professor",
        "curso": "Curs",
        "modulo": "Mòdul",
...
if "alumno_key" not in st.session_state:
  st.session_state.alumno_key = 0
if "reset_todo" not in st.session_state:
...
with st.sidebar:
  ...
  idioma_seleccionado = st.radio(
      "Idioma", ["Castellano", "Català"], horizontal=True, label_visibility="collapsed"
  )
      ...
      try:
        ...
        if response.status_code == 200:
          ...
          for i in range(1, len(df)):
            ...
            if (
                u_excel == usuario_in.strip()
                and p_excel == pass_in.strip()
            ):
              login_ok = True
              break
...
if opcion == T["menu_docs"]:
  ...
  # Seleccionamos el diccionario de datos según el idioma activo
  datos_activos = datos_es if lang == "es" else datos_ca
  ...
  with st.container(border=True):
          ...
          with st.expander(titulo):
            if (
              ...
              nuevo_text = st.text_area(
                  f"Editar {titulo}:",
                  value=datos_activos.get(titulo, ""),
                  height=150,
                  key=f"edit_{lang}_{titulo}",
              )
              if st.button(f"Guardar {titulo}", key=f"btn_{lang}_{titulo}"):
                if guardar_en_sheets(titulo, nuevo_text):
                  refrescar_app()
            st.markdown(datos_activos.get(titulo, ""), unsafe_allow_html=True)
    ...
    titulos_colab = ["Centros de formación", "Gremios", "Asociaciones"]

    for i, col in enumerate([col_c1, col_c2, col_c3]):
      with col:
        titulo = titulos_colab[i]
        with st.expander(titulo):
          if (
              st.session_state.autenticado
              and st.session_state.usuario_actual == "mzerojc"
            ...
            nuevo_text = st.text_area(
                ...
                value=datos_activos.get(titulo, ""),
                height=150,
                key=f"edit_col_{lang}_{titulo}",
            )
            if st.button(f"Guardar {titulo}", key=f"btn_col_{lang}_{titulo}"):
              if guardar_en_sheets(titulo, nuevo_text):
                refrescar_app()
          st.markdown(datos_activos.get(titulo, ""), unsafe_allow_html=True)
    ...
    with st.expander(titulo):
      if (
        ...
        temp_text = st.text_area(
            ...
            value=datos_activos.get(titulo, ""),
            height=150,
            key=f"input_{lang}_{titulo}",
        )
        if st.button(f"Guardar {titulo}", key=f"btn_save_{lang}_{titulo}"):
    ...
    with st.expander(titulo):
      if (
        ...
        nuevo_cont = st.text_area(
            ...
            value=datos_activos.get(titulo, ""),
            height=70,
            key=f"cont_{lang}_{titulo}",
        )
        if st.button(f"Guardar {titulo}", key=f"btn_save_cont_{lang}_{titulo}"):
      ...
      with st.expander(titulo):
        if (
          ...
          nuevo_text = st.text_area(
              ...
              value=datos_activos.get(titulo, ""),
              height=150,
              key=f"edit_part_{lang}_{titulo}",
          )
          if st.button(f"Guardar {titulo}", key=f"btn_part_{lang}_{titulo}"):
            if guardar_en_sheets(titulo, nuevo_text):
              refrescar_app()
        st.markdown(datos_activos.get(titulo, ""), unsafe_allow_html=True)
  ...
  if not st.session_state.autenticado:
    ...
    with st.container():
      ...
      profesor = c1.text_input(
          T["profesor"], key=f"f_prof_{st.session_state.reset_todo}"
      )

      opciones_cursos_display = (
          [f"{c['codigo_curso']} - {c['nombre_curso']}" for c in cursos_db]
          if cursos_db
          else ["MZ-M - Mecanizados"]
      )
      curso_seleccionado_full = c2.selectbox(
          T["curso"],
          opciones_cursos_display,
          key=f"f_cur_{st.session_state.reset_todo}",
      )
      curso_codigo_actual = (
          curso_seleccionado_full.split(" - ")[0]
          if " - " in curso_seleccionado_full
          else curso_seleccionado_full
      )

      modulos_filtrados = [
          m for m in modulos_db if m["curso_asociado"] == curso_codigo_actual
      ]
      opciones_modulos_display = (
          [f"{m['subcodigo']} - {m['descripcion']}" for m in modulos_filtrados]
          if modulos_filtrados
          else ["Selecciona un curso válido"]
      )
      modulo_seleccionado_full = c3.selectbox(
          T["modulo"],
          opciones_modulos_display,
          key=f"f_mod_{st.session_state.reset_todo}",
      )
      modulo_codigo_actual = (
          modulo_seleccionado_full.split(" - ")[0]
          if " - " in modulo_seleccionado_full
          else modulo_seleccionado_full
      )
      ...
      nivel = c4.text_input(
          T["nivel_bloque"],
          value=nivel_sugerido,
          key=f"f_niv_{st.session_state.reset_todo}",
      )
      alumno = c5.text_input(T["alumno"], key=f"f_alu_{st.session_state.alumno_key}")

    criterios = [
        "1. Tasa de eficiencia",
        "2. Precisión geométrica y mecánica",
        "3. Autonomía ejecutiva",
        "4. Índice de mermas",
        "5. Mantenimiento de utillaje y entorno",
        "6. Factor de desempeño temporal",
        "7. Resolución escenarios de prácticas",
        "8. Resolución escenarios de averías",
        "9. Precisión conceptual y terminología",
        "10. Seguridad y normativas",
        "11. Fiabilidad y compromiso operativo",
    ...
    for i, crit in enumerate(criterios):
      with cols[i % 4]:
        with st.container(border=True):
          ...
          with col_b:
            info_crit = descripciones_rubrica.get(
                crit,
                {
    ...
    if None not in notas.values() and alumno:
      nota_final = round(
          sum((notas[c] - 1) * 2.5 for c in criterios) / len(criterios), 1
      )
      res = (
          "SUSPENSO (Línea Roja)"
